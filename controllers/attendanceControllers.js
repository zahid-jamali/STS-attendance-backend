...
const adminCreatesAttendance = async (req, res) => {
  try {
    ...
    const newAttendance = new Attendance({
      ...
      workHours: exitTime
        ? (new Date(exitTime) - new Date(entryTime)) / (1000 * 60 * 60)
        : 0,
    });
  ...
  try {
    ...
    totalLeaves.forEach((leave) => {
      ...
      while (leaveStart <= leaveEnd) {
        ...
        leaveSet.set(key, {
          date: leaveStart.toISOString().split("T")[0],
          user: leave.user,
          Remarks: "On Leave",
        });
...
const getUsersAttendanceByDate = async (req, res) => {
  try {
    ...
    const leaveUsers = new Set(
      leaveRecords.map((leave) => leave.user.toString())
    );
    ...
    const result = users.map((user) => ({
      ...
      Name: user.Name,
      attendanceRecords:
        attendanceMap.get(user._id.toString()) ||
...
const markEntry = async (req, res) => {
  try {
    ...
    if (existingRecord) {
      return res
        .status(400)
        .json({ message: "Entry already marked for today" });
    }
...
const markExit = async (req, res) => {
  ...
  try {
    ...
    attendance.workHours =
      (attendance.exitTime - attendance.entryTime) / (1000 * 60 * 60);
...
module.exports = {
  markEntry,
  markExit,
  getUsersAttendanceByDate,
  getUserAttendance,
  adminCreatesAttendance,
};
